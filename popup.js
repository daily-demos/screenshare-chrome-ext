function createRoomAndShareScreen() {
    const baseUrlValue = document.getElementById('baseURL').value.trim().replace(/\/+$/, '');
    ...
    if (baseUrlValue) {
        fetch(`${baseUrlValue}/.netlify/functions/rooms/`, {
            ...
            .then(data => {
                if (data.url) {
                    const shouldShareScreen = document
                        .getElementById('screenShareCheckbox')
                    ...
                    document
                        ...
                        .textContent = 'Oops! A room url was not returned. Please check your base URL.'
                }
            })
            .catch(err => {
                console.warn(err);
                document
                    .getElementsByClassName('error')[0]
                    .textContent = 'Oops! That didn\'t work. Make sure you have a base URL in this format: https://<-your-subdomain->.netlify.app'
            });
    } else {
        document
            .getElementsByClassName('error')[0]
            .textContent = 'Make sure you have a base URL in this format: https://<-your-subdomain->.netlify.app'
    }
}
